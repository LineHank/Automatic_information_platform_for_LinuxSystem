<template>
  <div>
    <el-table border ref="multipleTable" :data="currentPagetableMessage" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" height="500">
    ...
    <el-table-column prop="password" label="系统密码" width="120"></el-table-column>
      <el-table-column prop="ping" label="网络状况" width="120" show-overflow-tooltip></el-table-column>
    ...
    <div style="text-align: center;margin-top: 30px;"><el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage" :page-sizes="[5, 10, 20, 40]" :page-size="pagesize" layout="total, sizes, prev, pager, next, jumper" :total="tableMessage.length"></el-pagination></div>
    ...
    <el-dialog title="编辑页面" :visible.sync="dialogTableVisible" width="30%" >
      ...
      </div><div style="margin:10px 0"><span style="font-size: 10px">系统登录密码：</span><el-input style="width: 200px;margin-left:11px" v-model="password" placeholder="请输入密码" size="small"></el-input></div>
...
//     props:['gettableMessage'],
     data() {return { currentPage:1, pagesize:10,currentPagetableMessage:[],id:"",ip:"",username:"",password:"",dialogTableVisible: false,button_remove_statu:true,tableStateType:'warning',tableMessage: [],multipleSelection: []}},
     methods: {
...
//         rows[index]["username"]="test"
       ...
       dialogdeicde(){
         let data = {"id":this.id,"ip": this.ip, "username": this.username, "password": this.password,};
         axios.post(`http://127.0.0.1:5000/dialogdeicde_update`, data).then(res => {console.log('res=>',this.dialogTableVisible=false,this.$message({message: '系统信息修改成功', type: 'success'}));}).catch(err => {console.log(this.$message.error('系统信息修改失败'))})
       },
       dialognodecide(){
